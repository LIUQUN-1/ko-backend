...
def delete_file_in_Neo4j(tx,userID,nodeID):
    query_word = ("MATCH (h) WHERE h.user_id=$name and id(h)=$nodeID DETACH Delete h")
    try:
        tx.run(query_word, name=int(userID), nodeID=int(nodeID))  # 执行Cypher查询
...
def main(request):
    ...
    nodeID_list=str(nodeIDs).split("hetian")
    ...
    for i in nodeID_list:
        data=session.write_transaction(search_file_in_Neo4j, userID,i)
        if len(data)<=0:
            node_no_exist+=1
            continue
        else:
            ...
            if result=="success":
                data = session.write_transaction(delete_file_in_Neo4j, userID, i)
                ...
                file_no_exist+=1
                continue
    success_delete=len(nodeID_list)-(file_no_exist+neo4j_fail+node_no_exist)
    if (file_no_exist+neo4j_fail+node_no_exist)<=0:
        return "成功删除"+str(success_delete)+"个文件"
    else:
        return "成功删除"+str(success_delete)+"个文件,"+"共有"+str(file_no_exist+neo4j_fail+node_no_exist)+"个文件删除失败"