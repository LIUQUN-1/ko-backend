...
def main(request):
    ...
    id = request.GET['id']
    all_urls=request.GET['urls']
    all_urls=str(all_urls).split("hetian")
    ...
    for i in all_urls:
        # print(i)
        result=search_in_neo4j(i)
        if result=="No data":
            special_flag=1
            neo4j_daoru(i,source)
            get_upload_html(i, source, bucket)
        else:
            result = json.loads(result)
            for j in result:
                file_name=j["n"]["name"]+".html"
                tmp = file_name.split('.')
                has_name = get_sha1_hash(tmp[0])
                upload_name = has_name + '/' + file_name
                bidui_path=get_sha1_hash('wikipage')[:2]+'/'+has_name
                ...
                # print(j["n"]["file_path"])
                # print(new_path)
                if "file_path" not in j["n"]:
                    continue
                if j["n"]["file_path"]=="":
                    special_flag=2
                    get_upload_html(i,source,bucket)
                    write_neo4j(j,new_path,source)
                else:
                    if new_path==j["n"]["file_path"]:
                        continue
                    else:
                        special_flag=3
                        bucket.remove_object("kofiles",j["n"]["file_path"])
                        get_upload_html(i,source,bucket)
                        write_neo4j(j,new_path,source)
    print(special_flag)
    if special_flag>0:
