...
def manage(results_place,sum_list,nodeid_list,nodes,linkid_list,links,flag):
    ...
    for res in relations:
        ...
        node1=res.nodes[1]
        tableSingle={}
        tableSingle["name"]=node0._properties["name"]
        tableSingle["type"]=res.type
        tableSingle["name1"]=node1._properties["name"]
        ...
        if res is not None:
            if res.id not in linkid_list:
                ...
                if "desc" in res._properties:
                    relDes=res._properties["desc"]


                    link_set["label"]={"show":True,"formatter":res.type,"fontSize":12,"desc": {"关系":relDes}}
                else:
                    print(1111111111)
                    if endResource==startResource:
                        try:
                            relDes="关系为同一来源："+startResource+"的内部关联，详情请前往："+node0._properties["url"]+"和"+node1._properties["url"]
                        except KeyError as e:
                            relDes="关系为同一来源："+startResource+"的内部关联"
                    else:
                        try:
                            relDes="关系为不同来源："+startResource+"--->"+endResource+"之间的关联，详情请前往："+node0._properties["url"]+"和"+node1._properties["url"]
                        except KeyError as e:
